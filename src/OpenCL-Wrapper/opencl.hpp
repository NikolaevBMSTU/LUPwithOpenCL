...
public:
	...
	inline void enqueue_kernel( cl::Kernel& kernel, const uint N) { 
	...
	inline void enqueue_kernel( cl::Kernel& kernel, const uint offset, const uint N) { 
		auto err = cl_queue.enqueueNDRangeKernel(kernel, cl::NDRange { offset }, cl::NDRange { N }, cl::NullRange);
		if (err != 0) throw std::runtime_error("OpenCL error code = " + err);
	}
};
...
public:
	inline Memory(Device& device, const ulong N, const bool allocate_host=true, const bool allocate_device=true, const T value=(T)0) {
		if(!device.is_initialized()) print_error("No Device selected. Call Device constructor.");
		if(N==0ull) print_error("Memory size must be larger than 0.");
		this->N = N;
	...
	inline Memory(Device& device, const ulong N, T* const host_buffer, const bool allocate_device=true) {
		if(!device.is_initialized()) print_error("No Device selected. Call Device constructor.");
		if(N==0ull) print_error("Memory size must be larger than 0.");
		this->N = N;
		allocate_device_buffer(device, allocate_device);
	...
	inline const ulong capacity() const { return N*sizeof(T); } // returns capacity of the buffer in Byte
